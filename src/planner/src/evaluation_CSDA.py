...
if __name__ == "__main__":
    ...
    for map_name in ["map1", "map2", "map3", "map4", "com1building"]:
        ...
        org_map = os.path.join("./org_map", map_name + ".pkl")
        res = 0.02 if map_name == "com1building" else 0.05
        inflation_ratio =  int(np.ceil(robot_diag/res/2)) +1
        with open(map_path, "rb") as fin:
            loaded_aug_map = pickle.load(fin)
        with open(org_map, "rb") as fin:
            loaded_org_map = pickle.load(fin)[3]
        for task in ["CSDA"]: #["DSDA", "CSDA", "DSPA"]:
            for goal in goals:
                
                for k in range(5):
                    planner = A_Star_Hybrid_Planner(loaded_aug_map.shape[0], loaded_aug_map.shape[1], res, inflation_ratio=inflation_ratio)
                    planner.set_goal(goal[0], goal[1],)
                    planner.map = loaded_org_map
                    planner.setup_map()
                    # planner.aug_map = loaded_aug_map.T
                    planner.generate_plan(init)
                    result = np.array(planner.action_seq)
                    filename = './%s_%s_%s_%s_%s.txt' % (task, map_name, goal[0], goal[1], k)
                    np.savetxt(filename, result, fmt="%.2e")
                    
                    try:
                        evaluator = Evaluator(
                            filename, loaded_aug_map, init, goal, res
                        )
